...
public class Base64 {
    ...
    private final static byte[] ALPHABET = { (byte) 'A', (byte) 'B', (byte) 'C', (byte) 'D', (byte) 'E', (byte) 'F', (byte) 'G', (byte) 'H', (byte) 'I', (byte) 'J', (byte) 'K', (byte) 'L',
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while ((b64is.read()) > 0) { // System.out.print( new String(
                                                // ""+(char)ib ) );
            }
        } // end try
        catch (Exception e) {
     ...
     * @since 1.3
     */
    public static byte[] encode3to4(byte[] threeBytes) {
     ...
     * @since 1.3
     */
    public static byte[] encode3to4(byte[] threeBytes, int numSigBytes) {
    ...
    /*
     * public static String encodeString2( String s ) { byte[] sb =
     * s.getBytes(); int sbL = sb.length; int sbL43 = sbL * 4 / 3; byte[]
     * outBuff = new byte[ ( sbL43 ) // Main 4:3 + ( (sbL % 3) > 0 ? 4 : 0 ) //
     * Account for padding + ( sbL43 / MAX_LINE_LENGTH ) ]; // New lines int d =
     * 0; int e = 0; int sbL2 = sbL - 2; int lineLength = 0; for( ; d < sbL2;
     * d+=3, e+=4 ) { encode3to4( sb, d, 3, outBuff, e );
     * 
     * lineLength += 4; if( lineLength == MAX_LINE_LENGTH ) { outBuff[e+4] =
     * NEW_LINE; e++; lineLength = 0; } // end if: end of line } // en dfor:
     * each piece of array
     * 
     * if( d < sbL ) { encode3to4( sb, d, sbL - d, outBuff, e ); e += 4; } //
     * end if: some padding needed
     * 
     * return new String( outBuff, 0, e ); } // end encode
     */
     ...
     * @since 1.3
     */
    public static byte[] decode4to3(byte[] fourBytes) {
    ...
    /*
     * public static byte[] decode2( String s ) { int sbL = s.length(); char[]
     * sb = new char[ sbL ]; s.getChars( 0, sbL, sb, 0 ); int sbL34 = sbL * 3 /
     * 4; byte[] outBuff = new byte[ sbL34 ]; // Upper limit on size of output
     * int outBuffPosn = 0;
     * 
     * byte[] b4 = new byte[4]; int b4Posn = 0; int i = 0; byte sbiCrop = 0;
     * byte sbiDecode = 0; for( i = 0; i < sbL; i++ ) { sbiCrop = (byte)(sb[i] &
     * 0x7f); // Only the low seven bits sbiDecode = DECODABET[ sbiCrop ];
     * 
     * if( sbiDecode >= WHITE_SPACE_ENC ) // White space, Equals sign or better {
     * if( sbiDecode >= EQUALS_SIGN_ENC ) { b4[ b4Posn++ ] = sbiCrop; if( b4Posn >
     * 3 ) { outBuffPosn += decode4to3( b4, 0, outBuff, outBuffPosn ); b4Posn =
     * 0;
     *  // If that was the equals sign, break out of 'for' loop if( sbiCrop ==
     * EQUALS_SIGN ) break; } // end if: quartet built
     *  } // end if: equals sign or better
     *  } // end if: white space, equals sign or better else {
     * System.err.println( "Bad Base64 input character at " + i + ": " + sb[i] +
     * "(decimal)" ); return null; } // end else: } // each input character
     * 
     * byte[] finalOut = new byte[ outBuffPosn ]; System.arraycopy( outBuff, 0,
     * finalOut, 0, outBuffPosn ); return finalOut; } // end decode
     */

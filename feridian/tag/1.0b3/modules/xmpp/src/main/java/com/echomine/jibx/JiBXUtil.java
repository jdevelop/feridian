...
public class JiBXUtil {
    ...
    private static IMarshaller iqPacketMapper;
    ...
    static {
        try {
            ...
            if (iqfactory == null && log.isWarnEnabled()) {
                ...
                int i = 0;
                for (String clas : iqfactory.getMappedClasses()) {
                    if (IQPacket.class.getName().equals(clas))
                        break;
                    else
                        i++;
                }
                iqPacketMapper = ((MarshallingContext)iqfactory.createMarshallingContext()).getMarshaller(i, IQPacket.class.getName());
            }
     ...
     * binding directory. This method requires a current parent marshalling
     ...
     * then nothing is done (no exception thrown either). This is normally used
     * to marshall extension elements that is not dependent on a parent writer.
     * 

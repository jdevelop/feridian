...
public abstract class BaseFileHandler implements FileHandler {
    ...
    /**
     * The method can be called to fire event whenever the file info (other
     * than the file size) has changed.  This will notify the listeners to
    ...
    /**
     * The method can be called to fire event whenever the file transfer
    ...
    /**
     * Fired before the transfer begins.  This gives the listener a chance to set things up for a transfer or any preliminary
     * processing.  The listener also has a chance to "veto" the transfer.  This means that if the listener finds that the
     * transfer should not start, it can throw a veto exception which essentially signals the file handler
     * to abort this transfer.  If the transfer is vetoed, whoever was notified
     * before the veto will get notified again that the transfer is "finished".
     * Thus, there is no need to send another file finished event.
     */
     ...
     * processing.  The listener also has a chance to "veto" the transfer.  This means that if the listener finds that the
     * transfer should not start, it can throw a veto exception which essentially signals the file handler
     * to abort this transfer.  This method will not catch the veto exception
     * and notify listeners that already processed the event to end the transfer.
     * This method is normally used when you need to propogate the file event
     * and will fire a transfer finished event later.
     */

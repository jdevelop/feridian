...
public class DataXFieldTest extends XMPPTestCase {
    ...
    public void testValues() throws Exception {
        String xml = "<field xmlns='jabber:x:data' type='jid-multi'><value>123@abc.com</value><value>345@abc.com</value></field>";
        StringReader reader = new StringReader(xml);
        DataXField field = new DataXField(DataXField.TYPE_JID_MULTI);
        field.addJIDValue(JID.parseJID("123@abc.com"));
        field.addJIDValue(JID.parseJID("345@abc.com"));
        JiBXUtil.marshallObject(writer, field);
        compare(reader);
        reader.reset();
        field = (DataXField) JiBXUtil.unmarshallObject(reader, DataXField.class);
    ...
    /**
     * tests marshalling/unmarshalling of optional data to make sure they get processed
     * properly
     */
    public void testMarshallOptionalData() throws Exception {
        String xml = "<field xmlns='jabber:x:data' type='text-single' label='label' var='var'><desc>description</desc><required/></field>";
        StringReader reader = new StringReader(xml);
    ...
    /**
     * tests that if a incoming message does not contain a field type (ie. null
     * or doesn't exist), then the field type should default to text-single.
     * Same is true if the field type is not a supported field type.
     */
    public void testUnmarshallMissingFieldType() throws Exception {
        String xml = "<field xmlns='jabber:x:data'/>";
        StringReader reader = new StringReader(xml);
        DataXField field = (DataXField) JiBXUtil.unmarshallObject(reader, DataXField.class);
        assertEquals(DataXField.TYPE_TEXT_SINGLE, field.getFieldType());
    }
    ...
    public void testJIDSingleField() throws Exception {
        ...
        field.setJIDValue(JID.parseJID("123@abc.com"));
    ...
    /**
     * makes sure that the description will always be non-null, no matter what
     * string is being set. This is for convenience since most of the time the
     * description retrieved should be for display purpose anyways.
     */

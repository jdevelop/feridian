 ...
 * acceptor will go back to receiving more connections.</p> <p>The acceptor offers both a synchronous and asynchronous version
 * of accept.  Running perpetually, this means that (1) synchronous accept will accept and handle one connection at a time and
 * (2) asynchronous accept will accept, hand it to a handler in a different thread, then go back to accept more
 * connections (ie. a daemon). Depending on which suits your situation, use the appropriate method.</p>
 * <p>Perpetual Acceptor also support firiing off of connection events.</p>
 * <p>NOTE: Using this class requires that the handler is multi-threading safe if asynchronous accept is used (doesn't apply
 * to synchronous accept).  This means for async accepts, the handler cannot contain any per-connection data.  Otherwise,
 * you'll get into data corruption because of multi-threading issues.</p>
 */

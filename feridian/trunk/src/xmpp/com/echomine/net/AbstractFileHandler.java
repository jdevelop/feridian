...
/**
 * <p>adds capability to use a FileModel.  Most of the time you should extend from this class unless it's absolutely
 * necessary that you implement the interface for some purpose.</p>
 * <p>A note on firing events:  There are a few events that can be fired.  Usually, the file info changed events are fired
 ...
 * to decide when to fire.</p> <p>Here's a suggested of using the event firing.  When receiving file info, fire
 ...
 * prematurely closed or errors occurred while transferring or handshaking, then fire fileTransferFinished(error status).</p>
 */
